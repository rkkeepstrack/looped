...
(container metadata only; decode happens per play); also hosts `supportedTypes`/`isSupported(url:)`,
the shared audio-type predicate.

## File map (18 Swift files)
...
| `Sources/looped/Views/ContentView.swift` | Root layout: animated collapsible **`Sidebar`** (private; import button + track list — `TrackRow`s with title/duration (fixed `Theme.trackRowHeight`), current row in `Theme.accent`, single click selects instantly (visual only), double-click → `LibraryViewModel.load` (no autoplay); width user-resizable by dragging the divider, clamped to `Theme.sidebarMinWidth…MaxWidth`, persisted via `@AppStorage "sidebarWidth"`; the track list is **hand-rolled** (plain `VStack`, not a native `List` — the NSTableView under a List draws its selection + drop indicator in the system accent with no recolor API, clashing with the theme): a per-row high-priority `DragGesture` (2pt threshold; wins arbitration over the row taps so pickup is immediate) reorders via `RowInsertion.dragGapIndex` → `LibraryViewModel.move`, a `TrackListDropDelegate` (private) handles external `.fileURL` drops via `RowInsertion.gapIndex` → `addDropped(urls:at:)`, and both draw a `Theme.insertionLine` (dimmed text-white) insertion indicator (1pt hairline + leading hollow dot, native-style) at the active gap; the empty state is a bordered drop zone) + a top-left toggle (`@AppStorage "sidebarOpen"`) + centered header (name + `currentTime | fileTime`) + waveform + bottom bar; hosts `KeyboardHandler`. Two drop zones: the sidebar list (insertion line) and the waveform (`Theme.waveformDropHighlight` wash while targeted; drop → `loadDropped`, loads immediately). |
...
| `Sources/looped/Views/Theme.swift` | Shared design tokens (`enum Theme`): warm-orange-on-black palette, waveform colors, and layout metrics (sidebar width, track-row height, panel corner/border). |
...
| `Sources/looped/Utils/RowInsertion.swift` | `enum RowInsertion`: pure gap-index math for the sidebar's hand-rolled reorder + drop insertion (uniform row height; gap 0 = above first row, count = below last, matching `Array.move` offsets). |
...
- `Tests/loopedTests/Utils/RowInsertionTests.swift` — sidebar reorder/drop gap-index math
  (boundaries, clamping, the displace-past-neighbor-center threshold).
